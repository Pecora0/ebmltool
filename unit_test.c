...
struct {
    ...
    {{""},         {.kind = RANGE_NONE}                                                                                   },
    {{"1"},        {.kind = RANGE_EXACT, .type = UINTEGER, .lo_uint = 1, .hi_uint = 1}                                    },
    {{"not 0"},    {.kind = RANGE_EXCLUDED, .type = UINTEGER, .lo_uint = 0, .hi_uint = 0}                                 },
    {{"not0"},     {.kind = RANGE_EXCLUDED, .type = UINTEGER, .lo_uint = 0, .hi_uint = 0}                                 },
    {{"1-8"},      {.kind = RANGE_UPLOW_BOUND, .type = UINTEGER, .lo_uint = 1, .lo_in = true, .hi_uint = 8, .hi_in = true}},
    {{"> 0x0p+0"}, {.kind = RANGE_LOWER_BOUND, .type = FLOAT, .lo_float = 0x0p+0, .lo_in = false}                      },
};
...
void range_print(EBML_Range r) {
    switch (r.kind) {
        case RANGE_UPLOW_BOUND:
            ...
            break;
        case RANGE_NONE:
            UNIMPLEMENTED("range_print: RANGE_NONE");
        case RANGE_EXACT:
            assert(r.type == UINTEGER);
